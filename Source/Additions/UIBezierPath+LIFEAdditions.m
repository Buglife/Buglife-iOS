...
static NSString * const kPenBezierPathDataString = @"";
...
+ (UIBezierPath *)life_penBezierPath
{
    static UIBezierPath *sBezierPath;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSData *data = [[NSData alloc] initWithBase64EncodedString:kPenBezierPathDataString options:0];
        sBezierPath = [NSKeyedUnarchiver unarchiveObjectWithData:data];
    });
    return [sBezierPath copy]; // return a copy because UIBezierPath is mutable
}
...
{
    const CGSize defaultSize = CGSizeMake(512, 512);
    UIBezierPath *path = [self life_penBezierPath];
    CGAffineTransform transform = CGAffineTransformMakeScale(size.width / defaultSize.width, size.height / defaultSize.height);
